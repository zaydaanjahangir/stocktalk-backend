...
def compute_and_store_vectors(tickers):
    with app.app_context():
        ...
        for stock in stocks:
            ...
            # Use the stock's own categories from the database
            categories = stock.categories or ["General"]
            category_embeddings = np.mean([model.encode(cat) for cat in categories], axis=0)

            # Combine only text-based embeddings
            stock_vector = np.concatenate([sector_embedding, name_embedding, category_embeddings])
